...
use base::Base;
use error::{DeserialError, DeserialErrorKind, ParseError, ParseErrorKind};
pub use link::Link;
...
impl Expr {
    pub fn from_bits<A: AsRef<[u8]>>(x: A) -> Self {
        bits!(x.as_ref().to_vec())
    }
    ...
    pub fn link(&self) -> Link {
        Link::make(&self.serialize())
    }

    #[must_use]
    pub fn hash(&self) -> Self {
        link!(self.position(), self.link())
    }
    ...
    pub fn deserialize(i: &[u8]) -> IResult<&[u8], Self, DeserialError<&[u8]>> {
        ...
        if is_atom {
            ...
            let data_bitlen = data_len
                .iter()
                .fold(0, |acc, &x| (acc * 256) + u64::from(x));
            ...
            match type_code {
                ...
                [0x02] => match String::from_utf8(data.to_owned()) {
                    Ok(s) => Ok((i, text!(s))),
                    Err(e) => Err(Error(DeserialError::new(
                        i,
                        DeserialErrorKind::Utf8Error(e),
                    ))),
                },
                [0x03] => {
...
pub fn parse_int(from: Span) -> IResult<Span, Expr, ParseError<Span>> {
    let (i, s) = alt((value(Sign::Minus, tag("-")), value(Sign::Plus, tag("+"))))(from)?;
...
pub fn parse_cons(from: Span) -> IResult<Span, Expr, ParseError<Span>> {
    let (upto, xs) = delimited(tag("("), separated_list0(multispace1, parse_expr), tag(")"))(from)?;
    Ok((upto, Expr::Cons(Some(Pos::from_upto(from, upto)), xs)))
}
pub fn parse_expr(i: Span) -> IResult<Span, Expr, ParseError<Span>> {
    alt((
        parse_bits, parse_raw, parse_text, parse_char, parse_int, parse_nat, parse_cons,
    ))(i)
}
...
pub mod tests {
    ...
    fn expr_test_cases() {
        ...
        let a_val = vec![
            52, 78, 86, 90, 12, 9, 43, 13, 0, 56, 30, 55, 26, 51, 91, 44, 23, 30, 91, 52, 86, 33,
            72, 41, 55, 56, 45, 98, 53, 40, 84, 60, 93, 84, 94, 48, 28, 30, 19, 63, 49, 16, 16, 95,
            50, 59, 49, 59, 52, 89, 27, 49, 90, 73, 38, 77, 12, 51, 61, 38, 28, 8, 11, 26, 58, 35,
            70, 37, 97, 90, 4, 64, 90, 20, 56, 79, 53, 73, 40, 52, 95, 43, 97, 73, 37,
        ];
        ...
        let a_ser = vec![
            1, 1, 2, 168, 52, 78, 86, 90, 12, 9, 43, 13, 0, 56, 30, 55, 26, 51, 91, 44, 23, 30, 91,
            52, 86, 33, 72, 41, 55, 56, 45, 98, 53, 40, 84, 60, 93, 84, 94, 48, 28, 30, 19, 63, 49,
            16, 16, 95, 50, 59, 49, 59, 52, 89, 27, 49, 90, 73, 38, 77, 12, 51, 61, 38, 28, 8, 11,
            26, 58, 35, 70, 37, 97, 90, 4, 64, 90, 20, 56, 79, 53, 73, 40, 52, 95, 43, 97, 73, 37,
        ];
        assert_eq!(a.serialize(), a_ser);
        assert_eq!(
            Expr::deserialize(&a_ser).unwrap(),
            (b"".as_ref(), a.clone())
        );
        let a = bits!(vec![]);
        assert_eq!(format!("{}", a), "~\"\"");
        ...
        assert_eq!(
            c.serialize(),
            vec![128, 3, 0, 1, 8, 0, 0, 1, 8, 0, 0, 1, 8, 0]
        );
        assert_eq!(
            Expr::deserialize(&vec![128, 3, 0, 1, 8, 0, 0, 1, 8, 0, 0, 1, 8, 0]).unwrap(),
            (b"".as_ref(), c.clone())
        );
