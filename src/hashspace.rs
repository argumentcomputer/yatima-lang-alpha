...
use hashexpr::{base::Base, link::Link, Expr};
...
pub fn hashspace_directory() -> PathBuf {
    let proj_dir = ProjectDirs::from("io", "yatima", "hashspace").expect(
...
impl Hashspace {
    ...
    pub fn put(&self, expr: &Expr) -> Link {
        ...
        match &self.dir {
            ...
            None => {
                ...
                self.put_local_storage(&link.to_string(), data64.as_str())
                    .unwrap();
    ...
    pub fn get_local_storage(&self, cid: &String) -> Result<String, String> {
        ...
        Ok(String::from(
            storage.get_item(cid.as_str()).unwrap().unwrap(),
        ))
    }
