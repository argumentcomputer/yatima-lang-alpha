use yatima_core::{
    ...
    anon::Anon,
    defs::{Def, Defs},
    package::{import_alias, Entry, Import, Index, Package},
    parse::{
        package::{parse_alias, parse_defs, parse_link, parse_with},
        span::Span,
        term::{parse_name, parse_space},
    },
    position::Pos,
};
...
use crate::file::{
    error,
    error::{FileError, FileErrorKind},
    store,
};

use std::{ffi::OsString, fs, path::PathBuf, rc::Rc};
...
use nom::{bytes::complete::tag, combinator::opt, sequence::terminated, Err, IResult};
...
pub fn entry_to_def(d: Entry) -> Result<Def, FileErrorKind> {
    ...
    let type_anon: Anon = Anon::from_ipld(&type_ipld).map_or_else(|e| Err(IpldError(e)), Ok)?;
    let term_ipld: Ipld =
        store::get(d.term_anon).map_or_else(|| Err(UnknownLink(d.term_anon)), Ok)?;
    let term_anon = Anon::from_ipld(&term_ipld).map_or_else(|e| Err(IpldError(e)), Ok)?;
    Def::unembed(d, type_anon, term_anon).map_or_else(|e| Err(EmbedError(Box::new(e))), Ok)
}
...
pub fn index_to_defs(i: &Index) -> Result<Defs, FileErrorKind> {
    ...
    for (n, cid) in &i.0 {
        let entry_ipld: Ipld = store::get(*cid).map_or_else(|| Err(UnknownLink(*cid)), Ok)?;
        let entry: Entry = Entry::from_ipld(&entry_ipld).map_or_else(|e| Err(IpldError(e)), Ok)?;
...
pub fn parse_import(
    ...
    move |i: Span| {
        ...
        let (i, _) = parse_space(i).map_err(error::convert)?;
        let (i, alias) = opt(terminated(parse_alias, parse_space))(i).map_err(error::convert)?;
        let alias = alias.unwrap_or_else(|| Rc::from(""));
        let (i, with) = opt(terminated(parse_with, parse_space))(i).map_err(error::convert)?;
        let (i, from) = opt(terminated(parse_link, parse_space))(i).map_err(error::convert)?;
        ...
        if let Some(from) = from {
            ...
            let (_, defs) = index_to_defs(&pack.index)
                .map_or_else(|e| Err(Err::Error(FileError::new(i, e))), |v| Ok((i, v)))?;
            let with: Vec<Rc<str>> = with.unwrap_or_else(|| defs.names());
            Ok((
                i,
                (
                    from,
            ...
            if has_path.is_some() {
                Err(Err::Error(FileError::new(
                    i,
                    FileErrorKind::ImportCycle(path),
                )))
            } else {
                let env = PackageEnv {
                    root: env.root.clone(),
...
pub fn parse_package(
    ...
    move |from: Span| {
        ...
        let file_name = env
            .path
            .file_name()
            .ok_or_else(|| Err::Error(FileError::new(i, FileErrorKind::MalformedPath)))?;
        ...
        let (i, _) = parse_space(i).map_err(error::convert)?;
        let (upto, (defs, index)) = parse_defs(input, defs)(i).map_err(error::convert)?;
