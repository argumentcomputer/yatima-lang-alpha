    ...
    pub fn decode(expr: Expr) -> Result<Self, DecodeError> {
        match expr {
            ...
            Atom(pos, Nat(x)) => {
                let idx: u64 = x
                    .try_into()
                    .map_err(|_| DecodeError::new(pos, vec![Expected::AnonTermVariU64]))?;
                Ok(Self::Vari(idx))
            }
            Atom(_, Link(link)) => Ok(Self::Link(link)),
            Atom(_, Bits(data)) => Ok(Self::Data(data)),
            Atom(..) => Err(DecodeError::new(
                expr.position(),
                vec![Expected::AnonTermAtom],
            )),
        }
    }
}
...
pub mod tests {
    ...
    pub fn arbitrary_anon_term(g: &mut Gen, ctx: u64) -> AnonTerm {
        let input: Vec<(i64, Box<dyn Fn(&mut Gen) -> AnonTerm>)> = vec![
            ...
            (
                1,
                Box::new(|g| Bind(Box::new(arbitrary_anon_term(g, ctx + 1)))),
            ),
