...
pub fn stringify(dag: DAGPtr, ini: u64, dep: u64) -> String {
    match dag {
        ...
        DAGPtr::App(link) => unsafe {
            ...
            format!(
                "@{}{}",
                stringify(*fun, ini, dep),
                stringify(*arg, ini, dep)
            )
        },
        DAGPtr::Let(link) => unsafe {
            ...
            format!(
                "L{}{}",
                stringify(*exp, ini, dep),
                stringify(*bod, ini, dep + 1)
            )
        },
...
pub fn check(
    ...
    match term.head {
            ...
            match typ.head {
                DAGPtr::All(mut link) => {
                    ...
                    if Uses::gth(infer_uses, *lam_uses) {
                        Err(CheckError::GenericError("Quantity mismatch.".to_owned()))
                    } else {
        ...
        DAGPtr::Let(_) => Err(CheckError::GenericError("TODO: Let typecheck".to_owned())),
        _ => {
            let depth = pre.len();
            ...
            if eq {
                ...
                Err(CheckError::GenericError("Type mismatch.".to_owned()))
            }
        }
    }
}
    ...
    match term.head {
        ...
        DAGPtr::Ref(mut link) => {
            ...
            let def = defs.get(def_link).ok_or_else(|| {
                CheckError::GenericError(format!("Undefined reference: {}, {}", nam, def_link))
            })?;
            let ctx = to_ctx(pre);
        ...
        DAGPtr::App(mut link) => {
            ...
            match fun_typ.head {
                DAGPtr::All(link) => {
                    ...
                    // copied)
                    let new_link = match fun_typ.head {
                    ...
                    if new_var.parents.is_some() {
                        ...
                        replace_child(DAGPtr::Var(NonNull::new(new_var).unwrap()), arg.head);
                    }
        ...
        DAGPtr::Cse(mut link) => {
            ...
            match exp_typ.head {
                DAGPtr::Slf(_) => {
                    let new_link = match exp_typ.head {
                        DAGPtr::Slf(link) => link,
                        _ => panic!("Error in the clone implementation"),
                    };
                    let Slf {
        ...
        DAGPtr::All(link) => {
            ...
            let dom = DAG::new(*dom);
            std::mem::drop(check(defs, pre.clone(), Uses::None, &dom, &mut typ)?);
            let dom_clone = dom;
            (*var).dep = pre.len() as u64;
        ...
        DAGPtr::Lit(mut link) => {
            ...
            Ok((to_ctx(pre), DAG::from_term(&infer_lit(&lit.clone()))))
        }
        DAGPtr::LTy(mut link) => {
        ...
        DAGPtr::Opr(_link) => {
            Err(CheckError::GenericError("TODO".to_owned()))
...
#[must_use]
pub const fn infer_lit(lit: &Literal) -> Term {
    match lit {
...
#[must_use]
pub const fn infer_lty(_: LitType) -> Term {
    Term::Typ(None)
}

// pub fn infer_opr(lit: PrimOp) -> Term {}

pub fn check_def(defs: &Defs, refs: &Refs, name: &str) -> Result<(), CheckError> {
