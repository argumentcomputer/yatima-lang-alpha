...
impl<'a, T> Iter<'a, T> {
    ...
    #[inline]
    #[must_use]
    pub const fn this(&self) -> Option<NonNull<DLL<T>>> {
        self.this
    }
}
...
impl<T> DLL<T> {
    ...
    pub fn add_after(&mut self, elem: T) {
        ...
        self.next
            .map_or((), |ptr| unsafe { (*ptr.as_ptr()).prev = new_next });
        self.next = new_next;
    }
    ...
    pub fn add_before(&mut self, elem: T) {
        ...
        self.prev
            .map_or((), |ptr| unsafe { (*ptr.as_ptr()).next = new_prev });
        self.prev = new_prev;
    }
    ...
    pub fn merge(&mut self, node: NonNull<Self>) {
        unsafe {
            ...
            self.prev
                .map_or((), |ptr| (*ptr.as_ptr()).next = Some(node));
            self.prev = Some(node);
        }
    }
...
impl<T: ToString> DLL<T> {
    pub fn as_string(&mut self) -> String {
        ...
        let head = &iter
            .next()
            .map_or(String::from(""), |head| head.to_string());
