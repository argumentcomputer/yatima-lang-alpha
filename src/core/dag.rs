...
impl DAG {
    #[must_use]
    pub const fn new(head: DAGPtr) -> Self {
        Self { head }
    }
    ...
    pub fn to_term(&self) -> Term {
        pub fn go(node: &DAGPtr, map: &mut HashMap<*mut Var, u64>, depth: u64) -> Term {
            match node {
                DAGPtr::Var(link) => {
                    let Var {
                        nam,
    ...
    pub fn from_subterm(
        ...
        match tree {
            ...
            Term::Lit(_, lit) => DAGPtr::Lit(alloc_val(Lit {
                lit: lit.clone(),
                parents,
            })),
            ...
            Term::Dat(_, bod) => unsafe {
                ...
                let bod =
                    Self::from_subterm(&**bod, depth, ctx, NonNull::new(bod_ref), rec_ref.clone());
                (*dat.as_ptr()).bod = bod;
                DAGPtr::Dat(dat)
            },
            ...
            Term::All(_, uses, nam, dom_img) => unsafe {
                ...
                let dom =
                    Self::from_subterm(&dom, depth, ctx, NonNull::new(dom_ref), rec_ref.clone());
                img_ctx.push_front(DAGPtr::Var(NonNull::new(var).unwrap()));
                let img = Self::from_subterm(
                    &img,
                    depth + 1,
                    img_ctx,
                    NonNull::new(img_ref),
            ...
            Term::App(_, fun_arg) => unsafe {
                ...
                let arg =
                    Self::from_subterm(&arg, depth, ctx, NonNull::new(arg_ref), rec_ref.clone());
                (*app.as_ptr()).fun = fun;
                (*app.as_ptr()).arg = arg;
                DAGPtr::App(app)
            },
            Term::Ann(_, typ_exp) => unsafe {
                ...
                let exp =
                    Self::from_subterm(&exp, depth, ctx, NonNull::new(exp_ref), rec_ref.clone());
                (*ann.as_ptr()).typ = typ;
                (*ann.as_ptr()).exp = exp;
                DAGPtr::Ann(ann)
            },
        ...
        fn go(
            ...
            let new_node = match node {
                ...
                DAGPtr::Lam(link) => unsafe {
                    ...
                    let lam =
                        alloc_lam(var.nam.clone(), var.dep, None, mem::zeroed(), Some(parents));
                    let Lam {
                        var: new_var,
                        bod: new_bod,
                        bod_ref,
                ...
                DAGPtr::Slf(link) => unsafe {
                    ...
                    let slf =
                        alloc_slf(var.nam.clone(), var.dep, None, mem::zeroed(), Some(parents));
                    let Slf {
                        var: new_var,
                        bod: new_bod,
                        bod_ref,
        ...
        fn format_parents(dll: Option<NonNull<Parents>>) -> String {
            match dll {
                Some(dll) => unsafe {
                    ...
                    let head = &iter
                        .next()
                        .map_or(String::from(""), |head| format_uplink(*head));
