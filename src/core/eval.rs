...
use crate::{
    core::{
        dag::*,
        dll::*,
        primop::{apply_bin_op, apply_una_op},
        upcopy::*,
    },
    term::{Def, Link},
};
    ...
    loop {
        match input {
            ...
            DAGPtr::All(link) => {
                ...
                let new_all = alloc_all(nam.clone(), *dep, None, *uses, *dom, *img, None);
                unsafe {
            ...
            DAGPtr::Let(link) => {
                ...
                let Var { nam, dep, .. } = old_var;
                let new_let = alloc_let(nam.clone(), *dep, None, *uses, *typ, *exp, *bod, None);
                unsafe {
    ...
    pub fn whnf(&mut self, defs: &HashMap<Link, Def>) {
        ...
        loop {
            match node {
                ...
                DAGPtr::Cse(link) => {
                    let mut body = unsafe { Self::new((*link.as_ptr()).bod) };
                    body.whnf(defs);
                    match body.head {
                        DAGPtr::Dat(body_link) => {
                            let Dat {
                                bod: single_body, ..
                            } = unsafe { body_link.as_ref() };
...
mod test {
    ...
    pub fn parse(i: &str) -> nom::IResult<Span, DAG, crate::parse::error::ParseError<Span>> {
    ...
    pub fn reduce_test() {
        ...
        let trm_str = &format!("(({n}) (({m}) ({id})) {id})", n = three, m = three, id = id,);
        println!("{}", trm_str);
