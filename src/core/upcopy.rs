...
pub fn clean_up(cc: &ParentPtr) {
    match cc {
        ...
        ParentPtr::LetTyp(mut link) | ParentPtr::LetExp(mut link) | ParentPtr::LetBod(mut link) => unsafe {
            ...
            ParentPtr::LetExp(link) => {
                ...
                match copy {
                    ...
                    None => {
                        ...
                        let new_let =
                            alloc_let(nam.clone(), *dep, None, *uses, new_child, *typ, *bod, None);
            ...
            ParentPtr::LetTyp(link) => {
                ...
                match copy {
                    ...
                    None => {
                        ...
                        let new_let =
                            alloc_let(nam.clone(), *dep, None, *uses, *exp, new_child, *bod, None);
                        (*link.as_ptr()).copy = Some(new_let);
                        let ptr: *mut Var = &mut (*new_let.as_ptr()).var;
                        for parent in DLL::iter_option(*var_parents) {
                            upcopy(DAGPtr::Var(NonNull::new(ptr).unwrap()), *parent)
            ...
            ParentPtr::LetBod(link) => {
                ...
                match copy {
                    ...
                    None => {
                        ...
                        let new_let =
                            alloc_let(nam.clone(), *dep, None, *uses, *exp, *typ, new_child, None);
                        (*link.as_ptr()).copy = Some(new_let);
                        let ptr: *mut Var = &mut (*new_let.as_ptr()).var;
                        for parent in DLL::iter_option(*var_parents) {
                            upcopy(DAGPtr::Var(NonNull::new(ptr).unwrap()), *parent)
