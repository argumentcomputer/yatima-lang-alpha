...
use rustyline::{error::ReadlineError, Cmd, Config, EditMode, Editor, KeyEvent};
...
use yatima_core::{
    check::{check_def, infer_term},
    dag::DAG,
    defs::Defs,
    parse::{span::Span, term::input_cid},
};
    ...
    loop {
        ...
        match readline {
            Ok(line) => {
                ...
                let res = command::parse_command(input_cid(line.as_str()), defs.clone())(
                    Span::new(&line),
                );
                match res {
                    Ok((_, command)) => match command {
                        ...
                        Command::Define(boxed) => {
                            ...
                            let res = check_def(&tmp_defs, n.clone());
                            match res {
                                Ok(res) => {
                                    defs = tmp_defs;
