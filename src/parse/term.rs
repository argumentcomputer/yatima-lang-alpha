use crate::{
    ...
    term::{LitType, PrimOp, Refs, Term, Uses},
};
...
pub fn is_valid_symbol_string(s: &str) -> bool {
    !s.is_empty()
        ...
        && !s.starts_with('\'')
        && !s.starts_with('#')
        && s.chars().all(is_valid_symbol_char)
}
...
pub fn parse_typed_definition(
    ...
    move |from: Span| {
        ...
        if refs.get(&nam).is_some() && !shadow {
            ...
            let (i, _) = parse_space(i)?;
            let (i, bs) = alt((
                terminated(parse_binders(refs.clone(), ctx.clone(), false), parse_space),
                success(Vec::new()),
            ))(i)?;
            let (i, _) = tag(":")(i)?;
            let (i, _) = parse_space(i)?;
...
pub fn parse_let(
    ...
    move |from: Span| {
        let (i, rec) = alt((value(true, tag("letrec")), value(false, tag("let"))))(from)?;
        let (i, _) = parse_space(i)?;
        let (i, uses) = parse_uses(i)?;
        let (i, (nam, exp, typ)) = parse_typed_definition(refs.clone(), ctx.clone(), rec, true)(i)?;
        let (i, _) = tag(";")(i)?;
        let (i, _) = parse_space(i)?;
        let mut ctx2 = ctx.clone();
...
pub fn parse_lit() -> impl Fn(Span) -> IResult<Span, Term, ParseError<Span>> {
    move |from: Span| {
        let (i, lit) = alt((parse_bits, parse_text, parse_char, parse_nat, parse_int))(from)?;
        let (upto, _) = throw_err(parse_builtin_symbol_end()(i), |_| {
            ParseError::new(
                i,
...
pub fn parse_opr() -> impl Fn(Span) -> IResult<Span, Term, ParseError<Span>> {
    move |from: Span| {
        let (i, op) = alt((
            ...
            alt((
                value(PrimOp::Shl, tag("#shl")),
                value(PrimOp::Shr, tag("#shr")),
            )),
            alt((
                value(PrimOp::Len, tag("#len")),
                value(PrimOp::Cat, tag("#cat")),
            )),
        ))(from)?;
        let (upto, _) = throw_err(parse_builtin_symbol_end()(i), |_| {
...
pub fn parse_expression(
    ...
    move |from: Span| {
        ...
        if has_ann.is_some() {
            let (i, typ) = context("type annotation", parse_apps(refs.clone(), ctx.clone()))(i)?;
...
pub mod tests {
    ...
    fn test_apps() {
        ...
        println!("res: {:?}", res);
        assert!(res.is_ok());
        let res = parse_apps(HashMap::new(), Vector::new())(Span::new("0d1 0d1 def"));
        println!("res: {:?}", res);
        assert!(res.is_ok());
    }
    ...
    fn test_cases() {
        use Term::*;
        let res = parse_expression(HashMap::new(), Vector::new())(Span::new("(Type :: Type)"));
        // println!("res: {:?}", res);
        assert!(res.is_ok());
        let res =
            parse_expression(HashMap::new(), Vector::new())(Span::new("(Type (Type Type)  )"));
        // println!("res: {:?}", res);
        assert!(res.is_ok());
       ...
       x (c x (c x (c x n)))))))))))))))",
        ...
        // println!("res: {:?}", res);
        assert!(res.is_ok());
        let res = parse_binder_full(HashMap::new(), Vector::new())(Span::new("(a b c: Type)"));
        // println!("res: {:?}", res);
        assert!(res.is_ok());
        let res = parse_binders(HashMap::new(), Vector::new(), true)(Span::new("Type Type"));
        println!("res: {:?}", res);
        assert!(res.is_ok());
        assert!(
            res.unwrap().1
        ...
        let res =
            parse_binders(HashMap::new(), Vector::new(), true)(Span::new("(A: Type) (a b c: A)"));
        // println!("res: {:?}", res);
        assert!(res.is_ok());
        assert!(
            res.unwrap().1
                == vec![
                    ...
                    (
                        Uses::Many,
                        String::from("a"),
                        Var(None, String::from("A"), 0)
                    ),
                    (
                        Uses::Many,
                        String::from("b"),
                        Var(None, String::from("A"), 1)
                    ),
                    (
                        Uses::Many,
                        String::from("c"),
                        Var(None, String::from("A"), 2)
                    ),
                ]
        );
        let res =
            parse_expression(HashMap::new(), Vector::new())(Span::new("\u{2200} Type -> Type"));
        // println!("res: {:?}", res);
        assert!(res.is_ok());
        let res = parse_expression(HashMap::new(), Vector::new())(Span::new(
            "\u{2200} (_ :Type) -> Type",
    ...
    fn term_parse_print(x: Term) -> bool {
        match parse_expression(test_refs(), Vector::new())(Span::new(&format!("{}", x))) {
            Ok((_, y)) => x == y,
            Err(e) => {
                println!("{}", x);
                println!("{}", e);
