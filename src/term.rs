use crate::{definition::Definition, hashspace::Hashspace, unembed_error::UnembedError};

use crate::{anon_term::AnonTerm, meta_term::MetaTerm};

pub use crate::core::{
    literal::{LitType, Literal},
    primop::PrimOp,
    uses::Uses,
};
...
impl PartialEq for Def {
    fn eq(&self, other: &Self) -> bool {
        self.name == other.name && self.typ_ == other.typ_ && self.term == other.term
    }
}
...
impl PartialEq for Term {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (Self::App(_, ta), Self::App(_, tb)) | (Self::Ann(_, ta), Self::Ann(_, tb)) => {
                ta.0 == tb.0 && ta.1 == tb.1
            }
...
impl Term {
    ...
    pub fn shift(self, inc: u64, dep: u64) -> Self {
        match self {
            ...
            Self::Lam(pos, nam, bod) => Self::Lam(pos, nam, Box::new((*bod).shift(inc, dep + 1))),
            Self::Slf(pos, nam, bod) => Self::Slf(pos, nam, Box::new((*bod).shift(inc, dep + 1))),
    ...
    pub fn embed(self) -> (AnonTerm, MetaTerm) {
        match self {
            ...
            Self::Lit(pos, lit) => (
                AnonTerm::Ctor(
                    String::from("lit"),
                    vec![AnonTerm::Data(lit.encode().serialize())],
                ),
                MetaTerm::Ctor(pos, vec![MetaTerm::Leaf]),
            ),
            Self::LTy(pos, lty) => (
                AnonTerm::Ctor(
                    String::from("lty"),
                    vec![AnonTerm::Data(lty.encode().serialize())],
                ),
                MetaTerm::Ctor(pos, vec![MetaTerm::Leaf]),
            ),
            Self::Opr(pos, opr) => (
                AnonTerm::Ctor(
                    String::from("opr"),
                    vec![AnonTerm::Data(opr.encode().serialize())],
                ),
                MetaTerm::Ctor(pos, vec![MetaTerm::Leaf]),
            ),
            Self::Typ(pos) => (
            ...
            Self::Lam(pos, name, body) => {
                ...
                (
                    AnonTerm::Ctor(String::from("lam"), vec![AnonTerm::Bind(Box::new(anon))]),
            ...
            Self::Slf(pos, name, body) => {
                let (anon, meta) = (*body).clone().embed();
                (
                    AnonTerm::Ctor(String::from("slf"), vec![AnonTerm::Bind(Box::new(anon))]),
                    MetaTerm::Ctor(pos, vec![MetaTerm::Bind(name, Box::new(meta))]),
                )
            }
            Self::App(pos, terms) => {
            ...
            (AnonTerm::Ctor(n, xs), MetaTerm::Ctor(pos, ys)) => {
                match (&n[..], xs.as_slice(), ys.as_slice()) {
                    ...
                    ("lit", [AnonTerm::Data(data)], [MetaTerm::Leaf]) => {
                        ...
                        let lit = Literal::decode(lit).map_err(UnembedError::DecodeError)?;
                        Ok(Self::Lit(*pos, lit))
                    }
                    ("lty", [AnonTerm::Data(data)], [MetaTerm::Leaf]) => {
                        let (_, lty) = hashexpr::Expr::deserialize(data)
                            .map_err(|_| UnembedError::DeserialError)?;
                        let lty = LitType::decode(lty).map_err(UnembedError::DecodeError)?;
                        Ok(Self::LTy(*pos, lty))
                    }
...
impl Def {
    #[must_use]
    pub const fn new(pos: Option<Pos>, name: String, docs: String, typ_: Term, term: Term) -> Self {
        Self {
            pos,
            name,
            docs,
    ...
    pub fn get_link(defn: Link, hashspace: &Hashspace) -> Result<Self, UnembedError> {
        ...
        let type_anon = hashspace
            .get(def.type_anon)
            .ok_or(UnembedError::UnknownLink(defn))?;
        let type_anon = AnonTerm::decode(type_anon).map_err(UnembedError::DecodeError)?;
        let term_anon = hashspace
            .get(def.term_anon)
            .ok_or(UnembedError::UnknownLink(defn))?;
        let term_anon = AnonTerm::decode(term_anon).map_err(UnembedError::DecodeError)?;
...
pub mod tests {
    ...
    pub fn arbitrary_name(g: &mut Gen) -> String {
        ...
        let mut s: String = s
            .chars()
            .filter(|x| {
                crate::parse::term::is_valid_symbol_char(*x) && char::is_ascii_alphabetic(x)
            })
            .collect();
        s.truncate(1);
        format!("_{}", s)
    ...
    fn arbitrary_lam(refs: Refs, ctx: Vector<String>) -> Box<dyn Fn(&mut Gen) -> Term> {
    ...
    fn arbitrary_slf(refs: Refs, ctx: Vector<String>) -> Box<dyn Fn(&mut Gen) -> Term> {
        Box::new(move |g: &mut Gen| {
            let n = arbitrary_name(g);
            let mut ctx2 = ctx.clone();
            ctx2.push_front(n.clone());
    ...
    fn arbitrary_let(refs: Refs, ctx: Vector<String>) -> Box<dyn Fn(&mut Gen) -> Term> {
        Box::new(move |g: &mut Gen| {
    ...
    fn arbitrary_all(refs: Refs, ctx: Vector<String>) -> Box<dyn Fn(&mut Gen) -> Term> {
        Box::new(move |g: &mut Gen| {
            let n = arbitrary_name(g);
            let u: Uses = Arbitrary::arbitrary(g);
            let mut ctx2 = ctx.clone();
    ...
    fn arbitrary_ref(refs: Refs, ctx: Vector<String>) -> Box<dyn Fn(&mut Gen) -> Term> {
        Box::new(move |_g: &mut Gen| {
            let mut rng = rand::thread_rng();
    ...
    fn arbitrary_app(refs: Refs, ctx: Vector<String>) -> Box<dyn Fn(&mut Gen) -> Term> {
        Box::new(move |g: &mut Gen| {
            Term::App(
                None,
                Box::new((
    ...
    fn arbitrary_ann(refs: Refs, ctx: Vector<String>) -> Box<dyn Fn(&mut Gen) -> Term> {
        Box::new(move |g: &mut Gen| {
            Term::Ann(
                None,
                Box::new((
    ...
    fn arbitrary_dat(refs: Refs, ctx: Vector<String>) -> Box<dyn Fn(&mut Gen) -> Term> {
        Box::new(move |g: &mut Gen| {
            Term::Dat(None, Box::new(arbitrary_term(g, refs.clone(), ctx.clone())))
        })
    }
    fn arbitrary_cse(refs: Refs, ctx: Vector<String>) -> Box<dyn Fn(&mut Gen) -> Term> {
        Box::new(move |g: &mut Gen| {
