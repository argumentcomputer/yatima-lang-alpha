    ...
    pub fn from_term_inner(
        ...
        match tree {
            ...
            Term::All(_, uses, nam, dom_img) => unsafe {
                ...
                let dom =
                    DAG::from_term_inner(&dom, depth, ctx, NonNull::new(dom_ref), rec_ref.clone());
            ...
            Term::App(_, fun_arg) => unsafe {
                ...
                let arg =
                    DAG::from_term_inner(&arg, depth, ctx, NonNull::new(arg_ref), rec_ref.clone());
                (*app.as_ptr()).fun = fun;
                (*app.as_ptr()).arg = arg;
                DAGPtr::App(app)
            },
            Term::Ann(_, typ_exp) => unsafe {
                ...
                let exp =
                    DAG::from_term_inner(&exp, depth, ctx, NonNull::new(exp_ref), rec_ref.clone());
                (*ann.as_ptr()).typ = typ;
                (*ann.as_ptr()).exp = exp;
                DAGPtr::Ann(ann)
            },
    ...
    pub fn from_subdag(
        ...
        let new_node = match node {
            ...
            DAGPtr::Lam(link) => unsafe {
                ...
                let lam = alloc_lam(var.nam.clone(), var.dep, None, mem::zeroed(), parents);
                let Lam {
                    var: new_var,
                    bod: new_bod,
                    bod_ref,
            ...
            DAGPtr::Slf(link) => unsafe {
                ...
                let slf = alloc_slf(var.nam.clone(), var.dep, None, mem::zeroed(), parents);
                let Slf {
                    var: new_var,
                    bod: new_bod,
                    bod_ref,
...
pub mod test {
    ...
    fn dag_def_iso(x: Def) -> bool {
        let y = DAG::to_term(&DAG::from_def(&x, Rc::from("test")), true);
        x.term == y
    }
}
