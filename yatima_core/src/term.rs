...
impl Term {
    ...
    pub fn embed(&self) -> (Anon, Meta) {
        match self {
            Self::Var(pos, name, idx) => (Anon::Var(*idx), Meta::Var(*pos, name.clone())),
            ...
            Self::Let(pos, rec, uses, name, terms) => {
                ...
                (
                    ...
                    Meta::Let(*pos, name.clone(), Box::new((typ_meta, exp_meta, bod_meta))),
                )
            }
        }
    }
    ...
    pub fn unembed(anon: &Anon, meta: &Meta) -> Result<Self, EmbedError> {
        match (anon, meta) {
            (Anon::Var(idx), Meta::Var(pos, nam)) => Ok(Self::Var(*pos, nam.clone(), *idx)),
    ...
    pub fn pretty(&self, rec: Option<&String>) -> String {
        ...
        fn alls(rec: Option<&String>, use_: &Uses, nam: &str, typ: &Term, bod: &Term) -> String {
            match bod {
...
pub mod tests {
    ...
    use crate::{parse::term::is_valid_symbol_char, tests::frequency};
    ...
    fn arbitrary_ref(defs: Defs, ctx: Vector<String>) -> Box<dyn Fn(&mut Gen) -> Term> {
        Box::new(move |_g: &mut Gen| {
            let mut rng = rand::thread_rng();
    ...
    fn arbitrary_lam(rec: bool, defs: Defs, ctx: Vector<String>) -> Box<dyn Fn(&mut Gen) -> Term> {
        Box::new(move |g: &mut Gen| {
            ...
            Lam(
                Pos::None,
                n,
                Box::new(arbitrary_term(g, rec, defs.clone(), ctx2)),
            )
        })
    }

    fn arbitrary_app(rec: bool, defs: Defs, ctx: Vector<String>) -> Box<dyn Fn(&mut Gen) -> Term> {
        Box::new(move |g: &mut Gen| {
            Term::App(
                Pos::None,
                Box::new((
    ...
    fn arbitrary_ann(rec: bool, defs: Defs, ctx: Vector<String>) -> Box<dyn Fn(&mut Gen) -> Term> {
        Box::new(move |g: &mut Gen| {
            Term::Ann(
                Pos::None,
                Box::new((
    ...
    fn arbitrary_slf(rec: bool, defs: Defs, ctx: Vector<String>) -> Box<dyn Fn(&mut Gen) -> Term> {
        Box::new(move |g: &mut Gen| {
            let n = arbitrary_name(g);
            let mut ctx2 = ctx.clone();
            ctx2.push_front(n.clone());
    ...
    fn arbitrary_dat(rec: bool, defs: Defs, ctx: Vector<String>) -> Box<dyn Fn(&mut Gen) -> Term> {
        Box::new(move |g: &mut Gen| {
            Term::Dat(
                Pos::None,
    ...
    fn arbitrary_cse(rec: bool, defs: Defs, ctx: Vector<String>) -> Box<dyn Fn(&mut Gen) -> Term> {
        Box::new(move |g: &mut Gen| {
            Term::Cse(
                Pos::None,
                Box::new(arbitrary_term(g, rec, defs.clone(), ctx.clone())),
    ...
    fn arbitrary_all(rec: bool, defs: Defs, ctx: Vector<String>) -> Box<dyn Fn(&mut Gen) -> Term> {
        Box::new(move |g: &mut Gen| {
            let n = arbitrary_name(g);
    ...
    fn arbitrary_let(rec: bool, defs: Defs, ctx: Vector<String>) -> Box<dyn Fn(&mut Gen) -> Term> {
        Box::new(move |g: &mut Gen| {
    ...
    pub fn arbitrary_term(g: &mut Gen, rec: bool, defs: Defs, ctx: Vector<String>) -> Term {
        let len = ctx.len();
        if len == 0 {
            ...
            frequency(
                ...
                vec![
                    ...
                    (
                        if rec { 100 } else { 0 },
                        Box::new(|_| Term::Rec(Pos::None)),
                    ),
                    (
                        100,
                        Box::new(|g| Term::Lit(Pos::None, Arbitrary::arbitrary(g))),
                    ),
                    (
                        100,
                        Box::new(|g| Term::LTy(Pos::None, Arbitrary::arbitrary(g))),
                    ),
                    (
                        100,
                        Box::new(|g| Term::Opr(Pos::None, Arbitrary::arbitrary(g))),
                    ),
