...
use crate::{
    dag::*, defs::Defs, dll::*, literal::Literal, position::Pos, term::Term, uses::*, yatima,
};
...
pub fn hash(dag: DAGPtr, dep: u64) -> Cid {
    ...
    DAG::dag_ptr_to_term(&dag, &mut map, dep, true)
        .embed()
        .0
        .cid()
}
        ...
        Term::Lam(pos, _, lam_bod) => {
            ...
            match typ.head {
                DAGPtr::All(all_link) => {
                    ...
                    bod_ctx.push((all_var.nam.clone(), *lam_uses, dom));
        ...
        Term::Dat(pos, dat_bod) => {
            ...
            match typ.head {
                DAGPtr::Slf(slf_link) => {
                    ...
                    let mut new_bod = DAG::new(DAG::from_subdag(*slf_bod, &mut map, Some(root)));
    ...
    match term {
        ...
        Term::Var(pos, nam, idx) => {
            ...
            let bind = ctx.get(dep).ok_or_else(|| {
                CheckError::UnboundVariable(*pos, error_context(&ctx), nam.clone(), dep as u64)
            })?;
            let subtract_use = (bind.1 - uses).ok_or_else(|| {
                CheckError::QuantityTooMuch(*pos, error_context(&ctx), nam.clone(), bind.1, uses)
            })?;
        ...
        Term::All(_, _, nam, dom_img) => {
            ...
            let mut dom_dag =
                DAG::from_term_inner(dom, ctx.len() as u64, Vector::new(), None, rec.clone());
            ctx.push((nam.clone(), Uses::None, &mut dom_dag));
        ...
        Term::Slf(_, nam, bod) => {
            ...
            let mut typ = DAG::from_term(&Term::Typ(Pos::None));
            let mut term_dag =
                DAG::from_term_inner(term, ctx.len() as u64, Vector::new(), None, rec.clone());
            ctx.push((nam.clone(), Uses::None, &mut term_dag));
...
pub fn check_def(defs: &Defs, name: Rc<str>) -> Result<Term, CheckError> {
    let def = defs
        .get(name.clone())
        .ok_or_else(|| CheckError::UndefinedReference(Pos::None, name.clone()))?;
    let (d, _, a) = def.embed();
    let def_cid = d.cid();
    let ast_cid = a.cid();
    let rec = Some((name.clone(), def_cid, ast_cid));
