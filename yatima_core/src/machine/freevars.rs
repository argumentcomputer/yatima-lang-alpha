...
impl FreeVars {
  ...
  #[inline]
  pub fn singleton(x: usize) -> FreeVars {
    FreeVars {
      list: vec![x]
    }
  }
  ...
  #[inline]
  pub fn from_vec(vec: &Vec<usize>) -> FreeVars {
  ...
  pub fn get(&self, val: usize) -> Option<usize> {
  ...
  pub fn bind_many(&mut self, arity: usize) {
    let mut i = 0;
    while i < self.list.len() {
      if self.list[i] < arity {
        self.list.truncate(i);
        break;
      }
      self.list[i] = self.list[i] - arity;
      i = i+1;
    }
  }
}
