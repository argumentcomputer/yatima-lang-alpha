...
pub fn parse_alias(i: Span) -> IResult<Span, Rc<str>, ParseError<Span>> {
    let (i, _) = tag("as")(i)?;
...
pub fn parse_with(i: Span) -> IResult<Span, Vec<Rc<str>>, ParseError<Span>> {
    let (i, _) = tag("(")(i)?;
...
pub fn parse_entry(
    input: Cid,
    defs: Defs,
) -> impl Fn(Span) -> IResult<Span, (Rc<str>, Def, Entry), ParseError<Span>> {
    move |from: Span| {
