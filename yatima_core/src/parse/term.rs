use crate::{
    defs::Defs,
    parse::{
        error::{throw_err, ParseError, ParseErrorKind},
        literal::*,
        op::parse_opr,
    },
    position::Pos,
    term::{LitType, Term, Uses},
};
...
pub fn parse_name(from: Span) -> IResult<Span, Rc<str>, ParseError<Span>> {
    let (i, s) = take_till1(|x| {
        char::is_whitespace(x) | (x == ':') | (x == ';') | (x == ')') | (x == '(') | (x == ',')
    })(from)?;
    let s: String = String::from(s.fragment().to_owned());
    if reserved_symbols().contains(&s) {
        Err(Err::Error(ParseError::new(
            from,
            ParseErrorKind::ReservedKeyword(Rc::from(s)),
        )))
    } else if s.starts_with('#') {
        Err(Err::Error(ParseError::new(
            from,
            ParseErrorKind::ReservedSyntax(Rc::from(s)),
        )))
    } else if is_numeric_symbol_string1(&s) | is_numeric_symbol_string2(&s) {
        Err(Err::Error(ParseError::new(
            from,
            ParseErrorKind::NumericSyntax(Rc::from(s)),
        )))
    } else if !is_valid_symbol_string(&s) {
        Err(Err::Error(ParseError::new(
            from,
            ParseErrorKind::InvalidSymbol(Rc::from(s)),
        )))
    } else {
        Ok((i, Rc::from(s)))
    }
...
pub fn parse_antiquote(
    ...
    move |from: Span| {
        let (upto, nam) = context("quasiquoted value", preceded(tag("#$"), digit1))(from)?;
...
pub fn parse_var(
    ...
    move |from: Span| {
        ...
        let is_rec_name = match rec.clone() {
            Some(rec_ref) => nam.clone() == rec_ref,
            _ => false,
        };
        if let Some((idx, _)) = ctx.iter().enumerate().find(|(_, x)| **x == nam.clone()) {
...
pub fn parse_lam(
    ...
    move |from: Span| {
        ...
        let (upto, bod) =
            parse_expression(input, defs.clone(), rec.clone(), ctx2, quasi.to_owned())(i)?;
        let pos = Pos::from_upto(input, from, upto);
        let trm = ns
            .iter()
            .rev()
...
pub fn parse_binder_full(
    ...
    ctx: Vector<Rc<str>>,
    quasi: Vector<Term>,
) -> impl Fn(Span) -> IResult<Span, Vec<(Uses, Rc<str>, Term)>, ParseError<Span>> {
    move |i: Span| {
        let (i, _) = tag("(")(i)?;
        let (i, _) = parse_space(i)?;
...
pub fn parse_binder(
    ...
    move |i: Span| {
        if nam_opt {
            alt((
                parse_binder_full(input, defs.clone(), rec.clone(), ctx.clone(), quasi.clone()),
                parse_binder_short(
                    input,
                    defs.to_owned(),
                    rec.clone(),
...
pub fn parse_binders(
    ...
    quasi: Vector<Term>,
    nam_opt: bool,
) -> impl FnMut(Span) -> IResult<Span, Vec<(Uses, Rc<str>, Term)>, ParseError<Span>> {
    move |mut i: Span| {
...
pub fn parse_all(
    ...
    move |from: Span| {
        ...
        let (upto, bod) =
            parse_expression(input, defs.to_owned(), rec.clone(), ctx2, quasi.to_owned())(i)?;
        let pos = Pos::from_upto(input, from, upto);
...
pub fn parse_type(input: Cid) -> impl Fn(Span) -> IResult<Span, Term, ParseError<Span>> {
    move |from: Span| {
...
pub fn parse_bound_expression(
    ...
    move |from: Span| {
        ...
        let (i, typ) =
            parse_expression(input, defs.clone(), rec.clone(), type_ctx, quasi.clone())(i)?;
        ...
        let (i, _) = parse_space(i)?;
        let (upto, trm) =
            parse_expression(input, defs.clone(), rec.clone(), term_ctx, quasi.clone())(i)?;
        let pos = Pos::from_upto(input, from, upto);
        let trm = bs.iter().rev().fold(trm, |acc, (_, n, _)| {
            Term::Lam(pos, n.clone(), Box::new(acc))
        });
...
pub fn parse_let(
    ...
    move |from: Span| {
        let (i, letrec) = alt((value(true, tag("letrec")), value(false, tag("let"))))(from)?;
        let (i, _) = parse_space(i)?;
        ...
        Ok((
            upto,
            Term::Let(pos, letrec, uses, nam, Box::new((typ, exp, bod))),
        ))
    }
}

pub fn parse_builtin_symbol_end() -> impl Fn(Span) -> IResult<Span, (), ParseError<Span>> {
    move |from: Span| {
        alt((
...
pub fn parse_lty(input: Cid) -> impl Fn(Span) -> IResult<Span, Term, ParseError<Span>> {
    move |from: Span| {
        let (i, lty) = alt((
...
pub fn parse_lit(input: Cid) -> impl Fn(Span) -> IResult<Span, Term, ParseError<Span>> {
    move |from: Span| {
        let (i, lit) = alt((
            parse_bits,
            parse_bytes,
...
pub fn parse_expression(
    ...
    move |from: Span| {
        let (i, trm) =
            parse_apps(input, defs.clone(), rec.clone(), ctx.clone(), quasi.clone())(from)?;
...
pub fn parse_apps(
    ...
    move |from: Span| {
        ...
        let (i2, fun) =
            parse_term(input, defs.clone(), rec.clone(), ctx.clone(), quasi.clone())(i2)?;
        let mut i = i2;
    ...
    move |i: Span| {
        context(
            ...
            alt((
                ...
                parse_self(input, defs.clone(), rec.clone(), ctx.clone(), quasi.clone()),
                parse_data(input, defs.clone(), rec.clone(), ctx.clone(), quasi.clone()),
                parse_case(input, defs.clone(), rec.clone(), ctx.clone(), quasi.clone()),
...
pub fn parse(i: &str, defs: Defs) -> IResult<Span, Term, ParseError<Span>> {
    parse_expression(input_cid(i), defs, None, Vector::new(), Vector::new())(Span::new(i))
}
pub fn parse_quasi(
    i: &str,
    defs: Defs,
...
pub mod tests {
    ...
    fn test_parse_binders() {
        ...
        fn test(
            nam_opt: bool,
            i: &str,
        ) -> IResult<Span, Vec<(Uses, Rc<str>, Term)>, ParseError<Span>> {
            parse_binders(
                input_cid(i),
                Defs::new(),
                None,
        ...
        fn test_full(i: &str) -> IResult<Span, Vec<(Uses, Rc<str>, Term)>, ParseError<Span>> {
            parse_binder_full(
                input_cid(i),
                Defs::new(),
                None,
            ...
            res.unwrap().1
                == vec![
                    ...
                    (
                        Uses::Many,
                        String::from("a"),
                        Var(Pos::None, String::from("A"), 0)
                    ),
                    (
                        Uses::Many,
                        String::from("b"),
                        Var(Pos::None, String::from("A"), 1)
                    ),
                    (
                        Uses::Many,
                        String::from("c"),
                        Var(Pos::None, String::from("A"), 2)
                    ),
                ]
        );
    }
