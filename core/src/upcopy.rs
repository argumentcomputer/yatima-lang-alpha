      ...
      match cc {
        ...
        ParentPtr::AppFun(link) => {
          ...
          match copy {
            ...
            None => {
              let new_app = alloc_app(*uses, new_child, *typ, *arg, None);
        ...
        ParentPtr::AppTyp(link) => {
          ...
          match copy {
            ...
            None => {
              let new_app = alloc_app(*uses, *fun, new_child, *arg, None);
              (*link.as_ptr()).copy = Some(new_app);
              for parent in DLL::iter_option(*parents) {
                stack.push((DAGPtr::App(new_app), *parent))
              }
        ...
        ParentPtr::AppArg(link) => {
          ...
          match copy {
            ...
            None => {
              let new_app = alloc_app(*uses, *fun, *typ, new_child, None);
              (*link.as_ptr()).copy = Some(new_app);
              for parent in DLL::iter_option(*parents) {
                stack.push((DAGPtr::App(new_app), *parent))
              }
        ...
        ParentPtr::LamTyp(link) => {
          ...
          match copy {
            ...
            None => {
              let new_lam = alloc_lam(*uses, new_child, *bod, None);
        ...
        ParentPtr::LamBod(link) => {
          ...
          match copy {
            ...
            None => {
              let new_lam = alloc_lam(*uses, *typ, new_child, None);
              (*link.as_ptr()).copy = Some(new_lam);
              for parent in DLL::iter_option(*parents) {
                stack.push((DAGPtr::Lam(new_lam), *parent))
              }
